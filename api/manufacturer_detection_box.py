...
baseUrl = "/api/manufacturerDetectionBox"
    ...
    if factory_id:  # 注意if条件中 "" 也是空, 按False处理
        ...
        if data:
            ...
            random_str = "".join(random.sample(
                'zyxwvutsrqponmlkjihgfedcba', 5))  # 获得随机数
            production_serial_number = factory_prefix + random_str + \
                str(get_current_milli_time())  # 为保证生产序列号唯一再加上时间戳
            # 根据生产厂商序列号生成二维码
            ...
            sql2 = "INSERT INTO manufacturer_detection_box(id,factory_id,production_serial_number,md5_result) " \
                   "VALUES('{}', '{}', '{}','{}')".format(
                       id, factory_id, production_serial_number, md5_result)
            db.execute_db(sql2)
...
def get_manufacturer_detection_box_by_id(id):
    if id:
        """获取某个检测盒信息"""
        sql = "SELECT * FROM manufacturer_detection_box WHERE id = '{}'".format(
            id)
        data = db.select_db(sql)
...
def manufacturer_detection_box_delete(id):
    if id:
        sql1 = "SELECT * FROM manufacturer_detection_box WHERE id = '{}'".format(
            id)
        data = db.select_db(sql1)
        if data:
            sql2 = "DELETE FROM manufacturer_detection_box WHERE id = '{}'".format(
                id)
            db.execute_db(sql2)
...
def is_valid_production_serial_number(production_serial_number):
    if production_serial_number:
        md5_result = get_md5(production_serial_number)
        sql1 = "SELECT * FROM manufacturer_detection_box WHERE md5_result = '{}'".format(
            md5_result)
        data = db.select_db(sql1)
        if data:
