...
def add_manufacturer():
    ...
    if factory_name and factory_prefix:  # 注意if条件中 "" 也是空, 按False处理
        sql1 = "SELECT * FROM manufacturer WHERE factory_name = '{}'".format(
            factory_name)
        data = db.select_db(sql1)
        if data:
            return jsonify({"code": 1002, "msg": "该厂商已经存在"})
        sql2 = "SELECT * FROM manufacturer WHERE factory_prefix = '{}'".format(
            factory_prefix)
        ...
        sql3 = "INSERT INTO manufacturer(id,factory_name,factory_prefix) " \
               "VALUES('{}', '{}', '{}')".format(
                   id, factory_name, factory_prefix)
        db.execute_db(sql3)
...
def get_manufacturer_by_factory_name(factory_name):
    if factory_name:
        """获取某个生产厂商信息"""
        sql = "SELECT * FROM manufacturer WHERE factory_name = '{}'".format(
            factory_name)
        data = db.select_db(sql)
